...
def parse_ao3_metadata(epub_file):
	...
	with zipfile.ZipFile(epub_file) as z:
		try:
			z_list=z.namelist()
			#first, we open "content.opf" to get the title, author and date. Those infos are already parsed by Calibre without our help, this will help up identify the book in Calibre's database
			with z.open("content.opf") as f:
				logging.debug("found content.opf")
				html=f.readlines()
				html=[x.decode("utf-8") for x in html]
				html="\n".join(html)
				...
				try:
					...
					title=soup.findAll("dc:title")[0].getText()
					author=soup.findAll("dc:creator")[0]
					author=author["opf:file-as"]
					date=soup.findAll("dc:date")[0].getText()
				identifier=(title,author,date)
				logging.debug("work identifier : "+(" ".join(identifier)))
		...
		metadata["tags"]=[]
		for column_name,column_list in {"characters": raw_data["Character"], "relationships":raw_data["Relationship"]}.items():
			#do this loop twice, once for "characters"+the list of characters. once for "relationships"+the list of relationships.
			#the variable "colum_name" is used to keep track of which iteration of the loop we're in.

			#column_name = "characters" or "relationships"
...
def fetch_value_id(table_name,value_real,create_missing=False,):
	...
	if table_name in ["tags"]:
		value_column_name="name" #calibre is not consistent here just to be annoying
...
if __name__=="__main__":
	...
	logging.debug("number of works to process :",len(works))
	...
	for work in works:
		logging.debug("starting processing on work "+work)


		try :
			data=parse_ao3_metadata(work)
			logging.debug("parse_ao3_metadata finished successfully")
		except AO3FormatError :
			logging.debug("AO3FormatError")
		...
		if data==False: #something went wrong collecting the data for that work
			continue
