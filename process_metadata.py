	...
	with zipfile.ZipFile(epub_file) as z:
		try:
			...
			with z.open("content.opf") as f:
				...
				author=soup.findAll("dc:creator")[0].getText()
...
def edit_calibre_database(identifier,metadata):
	...
	Identifier is a string "title+author+date of publication". It is used to find the book in calibre's database. If you have two books by the same author with the same title published the same day you will have a collision and one of the two books will not be correctly processed. I used to use the URI of the book for that, but AO3 updated the format of the epubs and now calibre can't find the URI by itself :( But well if this happens scold the author, not me, I mean come on.
	metadata must be a dictionnary produced by parse_ao3_metadata()
	"""


	#find the book from the identifier
	title,author,date=identifier.split("+",2)
	cursor.execute("SELECT id FROM books WHERE title='"+title+"'")
	#TODO here complete the identifying process
